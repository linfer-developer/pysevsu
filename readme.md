...
* [pysevsu](modules.md)
  * [core](pysevsu.core.md)
    * [interfaces](pysevsu.core.md#module-pysevsu.core.interfaces)
      * [`ExcelDataKey`](pysevsu.core.md#pysevsu.core.interfaces.ExcelDataKey)
      * [`WebsiteDataKey`](pysevsu.core.md#pysevsu.core.interfaces.WebsiteDataKey)
...
`ScheduleExcelParser` как [`sources.excel.File`](pysevsu.sources.md#pysevsu.sources.excel.File),
`ScheduleWebsiteParser` как [`sources.website.Structure`](pysevsu.sources.md#pysevsu.sources.website.Structure) .
...
#### *async* run(return_objects: bool = True, filters: dict[[WebsiteDataKey](pysevsu.core.md#pysevsu.core.interfaces.WebsiteDataKey) | [ExcelDataKey](pysevsu.core.md#pysevsu.core.interfaces.ExcelDataKey), Any] | None = None) → AsyncIterator[[Class](pysevsu.models.md#pysevsu.models.schemas.Class) | dict[[WebsiteDataKey](pysevsu.core.md#pysevsu.core.interfaces.WebsiteDataKey) | [ExcelDataKey](pysevsu.core.md#pysevsu.core.interfaces.ExcelDataKey), Any]]
...
Разрешенные импорты:
`ExcelFileDownloader` как [`engine.Downloader`](#pysevsu.core.engine.Downloader),
`ExcelFileProcessor` как [`engine.Processor`](#pysevsu.core.engine.Processor),
`Engine` как [`engine.Runner`](#pysevsu.core.engine.Runner),
`NetworkClient` как [`network.Client`](#pysevsu.core.network.Client),
`CorruptedExcelFileError` как `exceptions.CorruptedExcelFileError`,
`UnstableNetworkConnection` как `exceptions.UnstableNetworkConnection`,
`RequestPolicy` как [`interfaces.RequestPolicy`](#pysevsu.core.interfaces.RequestPolicy),
`WebsiteDataKey` как [`interfaces.WebsiteDataKey`](#pysevsu.core.interfaces.WebsiteDataKey),
`ExcelDataKey` как [`interfaces.ExcelDataKey`](#pysevsu.core.interfaces.ExcelDataKey),
`WebsiteIteratorInterface` как [`interfaces.ScheduleStructureIteratorProtocol`](#pysevsu.core.interfaces.ScheduleStructureIteratorProtocol),
`ExcelIteratorInterface` как [`interfaces.ExcelIteratorProtocol`](#pysevsu.core.interfaces.ExcelIteratorProtocol) .

## interfaces

Интерфейсы и базовые типы данных системы обработки расписания.

Модуль определяет публичные протоколы взаимодействия компонентов сетевого
очередей ([`AsyncQueue`](#pysevsu.core.interfaces.AsyncQueue), [`ThreadSafeQueue`](#pysevsu.core.interfaces.ThreadSafeQueue), [`MixedQueue`](#pysevsu.core.interfaces.MixedQueue)),
итераторов данных ([`ExcelIteratorProtocol`](#pysevsu.core.interfaces.ExcelIteratorProtocol),
...
* **Исключение:**
  ...
  * **UnstableNetworkConnection** – При превышении лимита сетевых ошибок.
...
### *class* Downloader(client: \_ClientProtocol, iterator: [ScheduleStructureIteratorProtocol](#pysevsu.core.interfaces.ScheduleStructureIteratorProtocol), queue: [AsyncQueue](#pysevsu.core.interfaces.AsyncQueue), filters: dict[[WebsiteDataKey](#pysevsu.core.interfaces.WebsiteDataKey) | [ExcelDataKey](#pysevsu.core.interfaces.ExcelDataKey), Any] | None = None)

Базовые классы: `_DownloaderProtocol`

Асинхронный компонент загрузки Excel файлов расписания из сети.

Планирует параллельные HTTP-запросы на основе элементов структуры сайта,
...
#### run() → AsyncIterator[tuple[bytes, dict[[WebsiteDataKey](#pysevsu.core.interfaces.WebsiteDataKey), Any]]]
...
### *class* Processor(executor: Executor, queue: [MixedQueue](#pysevsu.core.interfaces.MixedQueue), iterator_model: type[[ExcelIteratorProtocol](#pysevsu.core.interfaces.ExcelIteratorProtocol)], chunk_size: int = 50, filters: dict[[WebsiteDataKey](#pysevsu.core.interfaces.WebsiteDataKey) | [ExcelDataKey](#pysevsu.core.interfaces.ExcelDataKey), Any] | None = None)
...
#### run(file: bytes, website_data: dict[[WebsiteDataKey](#pysevsu.core.interfaces.WebsiteDataKey), Any] | None = None) → None
...
#### stream() → AsyncIterator[dict[[WebsiteDataKey](#pysevsu.core.interfaces.WebsiteDataKey) | [ExcelDataKey](#pysevsu.core.interfaces.ExcelDataKey), Any]]
...
#### start() → AsyncIterator[dict[[WebsiteDataKey](#pysevsu.core.interfaces.WebsiteDataKey) | [ExcelDataKey](#pysevsu.core.interfaces.ExcelDataKey), Any]]
...
### *class* Group(name: str, institute: str, course: int | None = None, degree: Degree | None = None)
...
### *class* Class(number: int, date: date, time: time, name: str, group: [Group](#pysevsu.models.schemas.Group), week: [Week](#pysevsu.models.schemas.Week), study_form: StudyForm | None = None, day: StudyWeekday | None = None, room: [Classroom](#pysevsu.models.schemas.Classroom) | None = None, teacher: [Teacher](#pysevsu.models.schemas.Teacher) | None = None, type_: str | None = None, subgroup: int | None = None)
...
Рекомендуемые импорты:
`ExcelFile` как [`excel.File`](#pysevsu.sources.excel.File),
`WebsiteStructure` как [`website.Structure`](#pysevsu.sources.website.Structure).
...
последовательностью словарей с ключами `ExcelDataKey` .
...
детерминизма (`ExcelDataKey`).

#### *property* name *: str*

Наименование рабочего листа.

#### with_filters(filters: dict[[ExcelDataKey](pysevsu.core.md#pysevsu.core.interfaces.ExcelDataKey), Any] | None = None) → [Worksheet](#pysevsu.sources.excel.Worksheet)
